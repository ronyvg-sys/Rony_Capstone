...
import json
import logging
import tiktoken # Rony added this line after the change made in the actual classs
...
async def ask_llm(q: Question, settings: Settings | None = None) -> Answer:
    ...
    enc = tiktoken.encoding_for_model(settings.model)
   # n_in = len(enc.encode([{"role": "user", "content": q.question}]))
    n_in = len(enc.encode(q.question)) 

    print(f"Number of token:{n_in}")
